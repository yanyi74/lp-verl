#!/usr/bin/env bash
# LP-GRPO w-regularization ablation: RAW mode (no normalization on w).
# Pair with run_lp_grpo_clip_30k_multinode.sh (same algo, different w mode).
#
# vs 原 lp_grpo_zpd_ema run 的唯一差别:
#   algorithm.lp_normalize_w=False
#   algorithm.lp_w_clip_lo=0.0
#   algorithm.lp_w_clip_hi=0.0
#  → w 不做任何规范化,直接用 raw 值进 advantage。
#   raw_w_mean ≈ 0.45,所以等效 LR ≈ 0.45× 名义值,天然 cooling。
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_raw_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_raw_30k_multinode.sh
...
# This script pins LP_W_MODE=raw (no normalization). See sibling clip script
# for the hard-clip ablation arm.
