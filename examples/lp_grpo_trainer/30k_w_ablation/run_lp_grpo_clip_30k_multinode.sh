#!/usr/bin/env bash
# LP-GRPO w-regularization ablation: CLIP mode (AWR/AWAC-style hard clip).
# Pair with run_lp_grpo_raw_30k_multinode.sh (same algo, different w mode).
#
# vs 原 lp_grpo_zpd_ema run 的唯一差别:
#   algorithm.lp_normalize_w=False
#   algorithm.lp_w_clip_lo=0.2
#   algorithm.lp_w_clip_hi=2.0
#  → w 不做 mean-norm,改成硬 clip 到 [0.2, 2.0],消除归一化系数随 raw_w_mean 漂移
#   带来的"弱信号被放大"问题。LP-GRPO 所有结构性创新(f_diff/f_prog/ZPD/EMA/bucket)保留。
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_clip_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_clip_30k_multinode.sh
...
# 跑在 baseline cluster 那组机器(跟 raw 实验并行,不抢节点)
...
# This script pins LP_W_MODE=clip with [0.2, 2.0] bounds.
# Override LP_W_CLIP_LO / LP_W_CLIP_HI to try different ranges.
LP_W_MODE=${LP_W_MODE:-clip}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_${LP_W_MODE}_30k_2node_$(date +%Y%m%d_%H%M)}
...
LP_W_CLIP_LO=${LP_W_CLIP_LO:-0.2}         # only used when LP_W_MODE=clip
LP_W_CLIP_HI=${LP_W_CLIP_HI:-2.0}         # only used when LP_W_MODE=clip
...
# Translate LP_W_MODE into the right algorithm.* args for w regularization.
case "${LP_W_MODE}" in
    norm)
        LP_W_ARGS=(
