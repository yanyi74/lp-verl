#!/usr/bin/env bash
# GRPO baseline on 17k uniform dataset (0/8~7/8 均匀分布), 2 nodes x 8 GPUs = 16 GPUs
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_grpo_baseline_17k_uniform_multinode.sh
#   On worker node: NODE_RANK=1 bash run_grpo_baseline_17k_uniform_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpo_baseline_17k_uniform_2node_$(date +%Y%m%d_%H%M)}
