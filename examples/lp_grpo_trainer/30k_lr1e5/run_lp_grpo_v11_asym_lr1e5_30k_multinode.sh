#!/usr/bin/env bash
# LP-GRPO v1.1 ASYMMETRIC (MILD): improving stronger than regressing.
#
# Formula:
...
#
# This ASYMMETRIC variant: alpha=1.5, beta=0.5 (ratio 3:1)
#   improving gets stronger amplification than regressing
#   both still get f_prog > 1 (not v1's extreme suppression)
#
# Expected bucket weights:
#   breakthrough  ~1.77   (improving, strongly boosted)
#   progress      ~1.52
#   plateau-mid   ~0.71
#   regressing    ~0.83   (still > 0, mild boost)
#   breakthrough/regressing ratio ~2.1x
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_v11_asym_lr1e5_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_v11_asym_lr1e5_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_v11_asym_a1.5_b0.5_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
...
# v1.1 asymmetric: 4 hyperparams (gamma, alpha, beta, lambda)
...
DATA=(
    algorithm.adv_estimator=lp_grpo_v11
    algorithm.use_kl_in_reward=False
    algorithm.lp_eps_p=0.05
    algorithm.lp_adaptive_n=False
    # v1.1 ASYMMETRIC: improving stronger (alpha=1.5) than regressing (beta=0.5)
    algorithm.lp_v11_gamma=0.5
    algorithm.lp_v11_alpha=1.5
    algorithm.lp_v11_beta=0.5
    algorithm.lp_v11_lambda=5.0
