#!/usr/bin/env bash
# LP-GRPO: lambda=5.0 + zpd_strength=1.0 + FINE-GRAINED NORMALIZE  @ lr=1e-5.
# Differs from run_lp_grpo_zpd1_lr1e5_30k_multinode.sh by ONE parameter:
#   LP_W_MODE : raw → norm
#
# With the updated core_algos.py, LP_W_MODE=norm now uses FINE-GRAINED normalize:
# divides by mean(w over groups with σ_g > eps), excluding stuck (A=0) groups
# from the denominator. Makes effective LR on useful samples exactly match
# baseline GRPO nominal LR. LP shape signal preserved (no clip).
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_zpd1_norm_lr1e5_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_zpd1_norm_lr1e5_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_zpd1_norm_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
...
# ----- LP-GRPO knobs: same as tuned except LP_ZPD_STRENGTH -----
LP_GAMMA=${LP_GAMMA:-0.5}
LP_LAMBDA=${LP_LAMBDA:-5.0}
LP_W_MAX=${LP_W_MAX:-5.0}
LP_EPS_P=${LP_EPS_P:-0.05}
LP_ZPD_STRENGTH=${LP_ZPD_STRENGTH:-1.0}   # ← key change: 0.5 → 1.0
LP_P0_EMA_ALPHA=${LP_P0_EMA_ALPHA:-0.05}
LP_W_MODE=${LP_W_MODE:-norm}
