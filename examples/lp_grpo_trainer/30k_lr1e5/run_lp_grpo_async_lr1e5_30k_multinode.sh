#!/usr/bin/env bash
# LP-GRPO Async (v_async): asymmetric per-rollout weighting, 0 hyperparameters.
#
# Differs from run_lp_grpo_zpd1_lr1e5_30k_multinode.sh:
#   - algorithm.adv_estimator: lp_grpo  →  lp_grpo_async
#   - All lp_* knobs removed (v_async is 0-hyperparameter)
#   - Same lr=1e-5, batch=128, N=8 (apples-to-apples comparison)
#
# Core formula (in compute_lp_grpo_async_outcome_advantage):
#   base_w = 4 * p_t * (1 - p_t)
#   delta  = clip(p_t - p_0, -0.8, 0.8)
#   w+     = base_w * (1 + delta * (1 - p_t))     for correct rollouts
#   w-     = base_w * (1 - delta * p_t)           for wrong rollouts
#   A_i_final = w_i * A_i_GRPO
#
# Key metrics to watch (in addition to critic/score/mean):
#   lp_async/active_groups       - groups with |delta| > 0.05 (should be 40-80)
#   lp_async/net_signal/mean_abs - magnitude of new directional signal
#   lp_async/w_plus/max          - should be < 1.8 (design bound)
#   lp_async/w_minus/min         - should be > 0 (math guarantee)
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_async_lr1e5_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_async_lr1e5_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_async_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
...
# v_async: 0 hyperparameters (no LP_GAMMA / LP_LAMBDA / LP_W_MAX / etc.)
...
DATA=(
    ...
    # v_async needs lp_eps_p for p_0 loader; other lp_* fields are unused.
    algorithm.lp_eps_p=0.05
