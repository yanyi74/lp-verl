#!/usr/bin/env bash
# Multi-node (2 nodes x 8 GPUs = 16 GPUs) baseline GRPO on the 30k dataset.
# Hyper-parameters aligned with the LP-GRPO run for fair comparison.
# LP bucket monitoring metrics (lp/bucket/*, lp/w/*, lp/f_prog/* etc.) are
# logged automatically since the trainer loads p_zero from the dataset.
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_grpo_baseline_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_grpo_baseline_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpo_baseline_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
