#!/usr/bin/env bash
# LP-GRPO: lambda=5.0 + zpd_strength=1.0 on the 30k dataset.
# Differs from run_lp_grpo_30k_multinode.sh (tuned) by ONE parameter:
#   LP_ZPD_STRENGTH : 0.5 → 1.0  (stronger ZPD suppression)
#
# Tests: does relaxing ZPD from 1.0 to 0.5 help? If this run ≈ tuned,
# zpd_strength matters less. If it's better, strong ZPD + strong lambda is
# the right combination.
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_zpd1_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_zpd1_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_zpd1_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
