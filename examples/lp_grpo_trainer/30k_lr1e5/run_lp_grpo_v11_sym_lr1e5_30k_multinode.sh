#!/usr/bin/env bash
# LP-GRPO v1.1 SYMMETRIC: fused ZPD+difficulty + tanh-bounded progress.
#
# Formula:
...
#
# This SYMMETRIC variant: beta = alpha = 1.5
#   → improving and regressing get EQUAL amplification
#   → contrast with v11_asym (beta < alpha)
#
# Key metrics to watch:
#   lp_v11/w/mean, lp_v11/w/max
#   lp_v11/f_prog/mean, lp_v11/f_prog/max
#   lp_v11/improving_count vs regressing_count
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_v11_sym_lr1e5_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_v11_sym_lr1e5_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_v11_sym_a1.5_b1.5_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
...
DATA=(
    ...
    # v1.1 SYMMETRIC: beta = alpha = 1.5
    algorithm.lp_v11_gamma=0.5
    algorithm.lp_v11_alpha=1.5
    algorithm.lp_v11_beta=1.5
