#!/usr/bin/env bash
# LP-GRPO: lambda=5.0 + gamma=0.3 + zpd_strength=0.5 + lr=1e-5  ("RATIO COMPRESS" config)
# Differs from run_lp_grpo_zpd1_lr1e5_30k_multinode.sh by TWO parameters:
#   LP_GAMMA         : 0.5 → 0.3   (less aggressive difficulty compression on mastered)
#   LP_ZPD_STRENGTH  : 1.0 → 0.5   (less aggressive p_t compression on extremes)
#
# Theory:
#   Old config has w_BT/w_MS ratio ≈ 38× → mastered nearly frozen (η_eff 7.5e-7)
#   → breakthrough updates interfere with mastered → mastered flow to regressing
#   → train reward -0.010 vs baseline, val crashes after step 180
#
#   New config: w_BT/w_MS ratio ≈ 11× (compressed 3.6×) but BT w almost unchanged (+2%)
#   mastered w boosted 264% (0.075 → 0.273), eff_LR jumps 3.64× to 2.73e-6
#   shape signal preserved, mastered protection restored.
#
# Predicted: train reward closes gap to baseline (or wins), val end > 0.626 (baseline),
#            mastered bucket grows like baseline, regressing flow stopped.
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_gamma03_zpd05_lr1e5_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_gamma03_zpd05_lr1e5_30k_multinode.sh
...
HEAD_IP=${HEAD_IP:-10.144.201.204}
...
PROJECT_NAME=${PROJECT_NAME:-verl_lp_grpo_30k}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_gamma03_zpd05_lr1e5_30k_2node_$(date +%Y%m%d_%H%M)}
...
# ----- LP-GRPO knobs: KEY CHANGES are LP_GAMMA and LP_ZPD_STRENGTH -----
LP_GAMMA=${LP_GAMMA:-0.3}                  # ← key change: 0.5 → 0.3 (protect mastered)
LP_LAMBDA=${LP_LAMBDA:-5.0}
LP_W_MAX=${LP_W_MAX:-5.0}
LP_EPS_P=${LP_EPS_P:-0.05}
LP_ZPD_STRENGTH=${LP_ZPD_STRENGTH:-0.5}   # ← key change: 1.0 → 0.5 (lift mastered ZPD compression)
LP_P0_EMA_ALPHA=${LP_P0_EMA_ALPHA:-0.05}
