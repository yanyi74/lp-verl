#!/usr/bin/env bash
# LP-GRPO | vision | vLLM rollout | FSDP training | GPU/NPU
...
unset _TRITON_BIN
# Prepend conda torch lib so libtorch_cuda.so is loaded from conda (2.8.0) rather than the
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_vl_8b_lp_grpo_vllm_fsdp_$(date +%Y%m%d_%H%M)}
