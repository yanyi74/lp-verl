#!/usr/bin/env bash
# Multi-node (2 nodes x 8 GPUs = 16 GPUs) baseline GRPO on the vppo36k dataset
# (pure VPPO_ViRL39K, 36,581 samples, schema-aligned to the 30k combined file).
# Hyper-parameters aligned with the LP-GRPO run for fair comparison.
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_grpo_baseline_vppo36k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_grpo_baseline_vppo36k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpo_baseline_vppo36k_2node_$(date +%Y%m%d_%H%M)}
