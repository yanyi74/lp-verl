#!/usr/bin/env bash
# [run_lp_grpo_zpd05_30k_multinode.sh]  LP-GRPO zpd=0.5
# LP-GRPO: lambda=5.0 + zpd_strength=0.5 on qwen2.5-vl-7b + qwen2_5_vl 30k dataset.
# Based on 30k_tuned/run_lp_grpo_30k_multinode.sh.
# Changes:
#   MODEL_PATH : Qwen3-VL-8B-Instruct → Qwen2.5-VL-7B-Instruct
#   TRAIN_FILE : mmrl+vppo qwen3 rollout → mmrl+vppo qwen2.5 rollout (30k)
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_zpd05_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_zpd05_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_tuned_qwen2_5vl_30k_2node_$(date +%Y%m%d_%H%M)}
