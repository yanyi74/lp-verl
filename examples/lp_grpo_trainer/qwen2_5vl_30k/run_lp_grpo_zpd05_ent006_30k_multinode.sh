#!/usr/bin/env bash
# [run_lp_grpo_zpd05_ent006_30k_multinode.sh]  LP-GRPO zpd=0.5 + entropy_coeff=0.06
# Based on run_lp_grpo_zpd05_30k_multinode.sh.
# Changes:
#   ENTROPY_COEFF : 0 → 0.06  (entropy regularization to encourage exploration)
#   PROJECT_NAME  : unchanged (verl_lp_grpo_qwen2_5vl_30k)
#   EXPERIMENT_NAME: lp_grpo_tuned_qwen2_5vl_30k → lp_grpo_zpd05_ent006_qwen2_5vl_30k
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_zpd05_ent006_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_zpd05_ent006_30k_multinode.sh
...
ENTROPY_COEFF=${ENTROPY_COEFF:-0.06}   # changed: 0 → 0.06
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_zpd05_ent006_qwen2_5vl_30k_2node_$(date +%Y%m%d_%H%M)}   # changed
