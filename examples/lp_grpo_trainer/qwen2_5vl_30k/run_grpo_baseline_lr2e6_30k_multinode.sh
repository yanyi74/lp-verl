#!/usr/bin/env bash
# [run_grpo_baseline_lr2e6_30k_multinode.sh]
# Vanilla GRPO baseline on qwen2.5-vl-7b + qwen2_5_vl 30k dataset.
# Hyper-parameters aligned with run_lp_grpo_zpd1_lr2e6_ema03_30k_multinode.sh
# so that any val/train acc gap is attributable to the LP-GRPO algorithm itself,
# not to LR / data / batch differences.
#
# LP bucket monitoring metrics (lp/bucket/*, lp/w/*, lp/f_prog/* etc.) are
# logged automatically since the trainer loads p_zero from the dataset, even
# though adv_estimator=grpo does not use them in the loss.
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_grpo_baseline_lr2e6_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_grpo_baseline_lr2e6_30k_multinode.sh
...
ACTOR_LR=${ACTOR_LR:-2e-6}                # matches LP-GRPO LR=2e-6 winner script
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpo_baseline_lr2e6_qwen2_5vl_30k_2node_$(date +%Y%m%d_%H%M)}
