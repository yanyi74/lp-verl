#!/usr/bin/env bash
# [run_lp_grpo_zpd05_revisual_30k_multinode.sh]  LP-GRPO zpd=0.5 + Revisual-R1-Coldstart
# Based on run_lp_grpo_zpd05_30k_multinode.sh.
# Changes:
#   MODEL_PATH : Qwen2.5-VL-7B-Instruct → Revisual-R1-Coldstart
#   PROJECT_NAME  : unchanged (verl_lp_grpo_qwen2_5vl_30k)
#   EXPERIMENT_NAME: lp_grpo_tuned_qwen2_5vl_30k → lp_grpo_zpd05_revisual_30k
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_zpd05_revisual_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_zpd05_revisual_30k_multinode.sh
...
MODEL_PATH=${MODEL_PATH:-/mnt/tidal-alsh01/dataset/perceptionVLM/yanyi/paper/model/Revisual-R1-Coldstart}   # changed
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_zpd05_revisual_30k_2node_$(date +%Y%m%d_%H%M)}   # changed
...
LP_ZPD_STRENGTH=${LP_ZPD_STRENGTH:-0.5}
