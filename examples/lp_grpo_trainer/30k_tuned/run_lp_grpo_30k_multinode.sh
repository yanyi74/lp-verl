#!/usr/bin/env bash
# LP-GRPO tuned on the 30k dataset. Pair with run_grpo_baseline_30k_multinode.sh
# for apples-to-apples comparison.
#
# Key changes vs. 30k/run_lp_grpo_30k_multinode.sh:
#   LP_P0_EMA_ALPHA : 0.15 → 0.05  (slower p_0 update, f_prog stays informative longer)
#   LP_LAMBDA       : 3.0  → 5.0   (stronger progress amplification)
#   LP_ZPD_STRENGTH : 1.0  → 0.5   (less aggressive suppression of early breakthroughs)
#   LP_W_MODE       : norm → raw    (no LR drift from mean normalization)
#   LP_GAMMA        : 0.5  (unchanged — keeps medium-progress w above 1)
#
# Formula:  w = (1-p_0)^γ × (1+λ·|Δp|) × (4·p_t·(1-p_t))^β
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_30k_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_30k_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_tuned_30k_2node_$(date +%Y%m%d_%H%M)}
...
# ----- LP-GRPO tuned knobs -----
LP_GAMMA=${LP_GAMMA:-0.5}                 # difficulty exponent — unchanged
LP_LAMBDA=${LP_LAMBDA:-5.0}               # progress amplification: 3.0 → 5.0
LP_W_MAX=${LP_W_MAX:-5.0}
LP_EPS_P=${LP_EPS_P:-0.05}
LP_ZPD_STRENGTH=${LP_ZPD_STRENGTH:-0.5}   # ZPD strength: 1.0 → 0.5
LP_P0_EMA_ALPHA=${LP_P0_EMA_ALPHA:-0.05}  # EMA alpha: 0.15 → 0.05 (most important)
LP_W_MODE=${LP_W_MODE:-raw}               # norm → raw (no LR drift)
