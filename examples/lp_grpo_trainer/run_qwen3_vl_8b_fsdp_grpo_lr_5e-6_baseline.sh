#!/usr/bin/env bash
# GRPO baseline | vision | vLLM rollout | FSDP training | GPU/NPU
# Qwen3-VL-8B with standard GRPO. All hyper-parameters aligned with
# run_qwen3_vl_8b_fsdp.sh (LP-GRPO) for a fair comparison.

set -xeuo pipefail

_TRITON_BIN=/root/miniconda3/envs/verl/lib/python3.12/site-packages/triton/backends/nvidia/bin
...
unset _TRITON_BIN
export LD_LIBRARY_PATH=/root/miniconda3/envs/verl/lib/python3.12/site-packages/torch/lib:${LD_LIBRARY_PATH:-}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_vl_8b_fsdp_grpo_lr_5e-6_baseline_$(date +%Y%m%d_%H%M)}
