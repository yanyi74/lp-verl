#!/usr/bin/env bash
# LP-GRPO final algorithm on the 30k dataset. Pair with run_grpo_baseline_30k_multinode.sh
...
# LP_W_MODE selects how the per-group LP weight w is regularized before being
# multiplied into the advantage:
#   norm  - mean-normalize so mean(w)=1 (legacy; can amplify weak signals as raw_w_mean drifts)
#   raw   - use w as-is, no normalization (effective LR ~ raw_w_mean, ~0.45x baseline)
#   clip  - hard-clip w to [LP_W_CLIP_LO, LP_W_CLIP_HI] (AWR/AWAC-style, no drift)
