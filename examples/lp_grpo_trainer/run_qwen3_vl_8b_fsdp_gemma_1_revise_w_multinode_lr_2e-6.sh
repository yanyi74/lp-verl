#!/usr/bin/env bash
# Multi-node (2 nodes x 8 GPUs = 16 GPUs) version of
# run_qwen3_vl_8b_fsdp_gemma_1_revise_w.sh
#
# Usage (run on BOTH nodes, no SSH automation needed):
#   On head node (this machine, 10.144.205.233):
#       NODE_RANK=0 bash run_qwen3_vl_8b_fsdp_gemma_1_revise_w_multinode.sh
#   On worker node (10.144.200.212):
#       NODE_RANK=1 bash run_qwen3_vl_8b_fsdp_gemma_1_revise_w_multinode.sh
#
# The head bootstraps a Ray cluster, waits for the worker to join, then launches
# training. The worker only starts a Ray worker process and blocks.
...
# If your cluster uses a specific NIC for cross-node traffic, set it here.
# 10.144.x.x looks like the inter-node ethernet — adjust as needed.
...
if [ "${NODE_RANK}" = "0" ]; then
    ...
    # ray stop often leaves raylet/gcs_server/redis behind. Force-kill, then wipe state.
    ...
    ray start --address=${HEAD_IP}:${RAY_PORT} \
              --num-gpus=${NDEVICES_PER_NODE} \
              --block
    # --block keeps the process alive. Training is launched only on the head.
    exit 0
fi
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_vl_8b_fsdp_gemma_1_sym_kl_2node_$(date +%Y%m%d_%H%M)}
