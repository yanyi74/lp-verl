#!/usr/bin/env bash
# LP-GRPO final algorithm: "Learning Progress GRPO with signal-aware resource
# allocation". Three orthogonal axes:
#
#   w = (1 - p_0)^γ × (1 + λ·|Δp|) × (4·p_t·(1-p_t))^β
#       └─ difficulty ┘   └ progress ┘    └── signal/allocation ──┘
#
# Three mechanisms vs your original (1-p_0)^γ × (1+λ·KL):
#   1) |Δp| replaces KL: linear, bounded in [0,1], numerically stable, robust
#      to N=8 binomial noise (KL log term blows up near p∈{0,1}).
#   2) Two-sided ZPD: 4·p_t·(1-p_t) ∈ [0,1]. Implements signal-aware resource
#      allocation — mastered (p_t≈1) and full-fail (p_t≈0) get auto-compressed
#      so their budget flows to progress / breakthrough / plateau-medium.
#   3) Per-step EMA on p_0 (α=0.15): replaces the epoch-end整批覆盖, lets the
#      |Δp| progress signal actually accumulate across epochs.
#
# Rollout-side adaptive_n is INTENTIONALLY OFF here: we want to isolate whether
# the algo changes alone work. Once validated, layer adaptive_n on top.
#
# Three core knobs (γ, λ, β) + 1 EMA (α). Bucket boost/penalty kept at 1.0
# (let the three axes do the work; no extra per-bucket steering).
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_qwen3_vl_8b_fsdp_zpd_ema_multinode.sh
#   On worker node: NODE_RANK=1 bash run_qwen3_vl_8b_fsdp_zpd_ema_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3_vl_8b_fsdp_zpd_ema_2node_$(date +%Y%m%d_%H%M)}
...
# ----- LP-GRPO algorithm-side knobs (final v: |Δp| + ZPD + EMA) -----
LP_GAMMA=${LP_GAMMA:-0.5}              # difficulty awareness exponent
LP_LAMBDA=${LP_LAMBDA:-3.0}            # progress amplification (linear in |Δp|)
LP_W_MAX=${LP_W_MAX:-5.0}              # unused now (kept for backward compat / metric only)
...
LP_P0_EMA_ALPHA=${LP_P0_EMA_ALPHA:-0.15}      # slow EMA on p_0 (per-step in advantage fn)
# Bucket factors all 1.0 (no extra boost/penalty) — let the three axes do the work.
