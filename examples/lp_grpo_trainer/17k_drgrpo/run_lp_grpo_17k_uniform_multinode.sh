#!/usr/bin/env bash
# LP-GRPO on 17k uniform dataset, 2 nodes x 8 GPUs.
# Same algorithm as the original 17k/run_lp_grpo, but with aggressive LP hyperparams:
#   LP_GAMMA  = 1.5  (was 0.5; sharper difficulty discrimination)
#   LP_LAMBDA = 10.0 (was 3.0;  actually activates progress amplification)
# Pair with run_lp_drgrpo_17k_uniform_multinode.sh to isolate the effect of
# turning off GRPO's std normalization (the only difference between the two).
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_grpo_17k_uniform_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_grpo_17k_uniform_multinode.sh
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_grpo_17k_uniform_2node_$(date +%Y%m%d_%H%M)}
