#!/usr/bin/env bash
# LP-GRPO on Dr.GRPO base (no std normalization) on 17k uniform dataset, 2 nodes x 8 GPUs.
# Pair with run_drgrpo_baseline_17k_uniform_multinode.sh as the apples-to-apples baseline.
# Key differences vs the original lp_grpo 17k script:
#   - algorithm.norm_adv_by_std_in_grpo=False  (Dr.GRPO base, LP is the sole difficulty knob)
#   - LP_GAMMA  defaults to 1.5  (was 0.5; sharper difficulty discrimination)
#   - LP_LAMBDA defaults to 10.0 (was 3.0;  actually activates progress amplification)
#
# Usage:
#   On head node:   NODE_RANK=0 bash run_lp_drgrpo_17k_uniform_multinode.sh
#   On worker node: NODE_RANK=1 bash run_lp_drgrpo_17k_uniform_multinode.sh
...
PROJECT_NAME=${PROJECT_NAME:-verl_17k_uniform}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-lp_drgrpo_17k_uniform_2node_$(date +%Y%m%d_%H%M)}

TRAIN_FILE=${TRAIN_FILE:-/mnt/tidal-alsh01/dataset/perceptionVLM/yanyi/paper/data/combined/filtered/strategy2_uniform17k/mmrl6914_vppo10118_lp_grpo.parquet}
TEST_FILE=${TEST_FILE:-/mnt/tidal-alsh01/dataset/perceptionVLM/yanyi/data/geo3k/test.parquet}

LP_GAMMA=${LP_GAMMA:-1.5}
LP_LAMBDA=${LP_LAMBDA:-10.0}
...
DATA=(
    ...
    algorithm.lp_adaptive_n=True
    algorithm.lp_n_min=2
